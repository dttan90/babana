...
class Home extends BaseController
{
    ...
    public function saveOrder()
    {
        ...
        if ($request->is('post')) {
            ...
            // get and set grid data
            $gridData = $this->request->getVar('gridData');
        }
        // return view('save_order');
        return json_encode(array('status' => 'true', 'name2' => $table_order_name), JSON_UNESCAPED_UNICODE);
    }
    ...
    public function getAreaId()
    {
        ...
        if ($request->is('post')) {
            ...
            if (!empty($table_id) ) {
                ...
                $areaItem = $AreaModel->readAreaItem(array('area_id' => $area_id));
                $area_name = !empty($areaItem) ? $areaItem['area_name'] : '';
            }
            
        }

        return json_encode(array('area_id' => $area_id, 'area_name' => $table_id), JSON_UNESCAPED_UNICODE);
