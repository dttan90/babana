...
class Home extends BaseController
{
    ...
    public function saveOrder()
    {
        ...
        $request = \Config\Services::request();
        if ($request->is('post')) {
            $data = $this->request->getVar('formData');
            $data = json_decode($data, true);

            
            // $test = is_array($data) ? "array" : 'json';
            $test = array();
            ...
            // $name2 = $data['formData']['total'];

            // print_r($name2); exit();
         
        }
        // return view('save_order');
        return json_encode(array('status' => 'true', 'name2' => 'test'), JSON_UNESCAPED_UNICODE);
    }
