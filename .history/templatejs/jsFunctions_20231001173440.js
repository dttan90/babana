                                ...
                                align: "around", width: "900px", padding: "20px", cols: [
                                    {
                                        rows: [
                                            ...
                                            {
                                                type: "combo", name: "area_name", label: "Khu vực", placeholder: "Chọn Khu vực", listHeight: "100px", newOptions: true, labelWidth: "270px", padding: "10px", helpMessage: "Xóa bỏ dữ liệu đang có và chọn lại các tùy chọn",
            ...
            addForm.events.on("click", function (name, new_value) {
                if (name == "cancel") {
                    ...
                    if (gridData.length == 0) {
                        dhx.alert({ header: "Đơn hàng", text: "Đơn hàng bạn tạo chưa có món nào", buttonsAlignment: "center", });
                        return
                    } else {

                        if (!formData.table_order_name) {
                            dhx.alert({ header: "Đơn hàng", text: "Bạn chưa chọn Bàn của đơn hàng", buttonsAlignment: "center", });
                        } else if (!formData.area_name) {
                            dhx.alert({ header: "Đơn hàng", text: "Bạn chưa chọn Bàn của đơn hàng", buttonsAlignment: "center", });
                        } else if (!formData.count_orders) {
                            dhx.alert({ header: "Đơn hàng", text: "Bạn chưa nhập số món (khác nhau)", buttonsAlignment: "center", });
                        } else {
                            var obj = {
                                formData: formData,
                                gridData: gridData
                    ...
                    //         console.log(`save data ok: ${JSON.stringify(result)}`);
                    //     },
                    //     error: function () {
                    ...
                    // $.ajax({
                    //     method: "POST", url: "saveOrder", data: { data: JSON.stringify(obj) }
                    // })
                    //     .done(function (data) {
