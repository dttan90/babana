        ...
        getWindowData(function (data) {
            ...
            addForm.events.on("click", function (name, new_value) {
                if (name == "cancel") {
                    ...
                    var formData = JSON.stringify(addForm.getValue());
                    var gridData = rightGrid.data.serialize();
                    console.log(`formData: ${formData}`);
                    console.log(`gridData: ${JSON.stringify(gridData)}`);

                    $.ajax({ method: "POST", url: "saveOrder", data: { formData: formData, gridData: gridData }, dataType: "json"
                    })
                        .done(function (data) {
                            console.log(`save data ok: ${JSON.stringify(data)}`);
                        })
                        .fail(function () {
                            alert("Không lấy được dữ liệu từ hệ thống??");
                        })
